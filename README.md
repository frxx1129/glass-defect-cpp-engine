...
玻璃缺陷检测系统（检测 E 边缘异常 / B 崩边 / L 裂纹 / Q 缺角）的 **C++ 实现**，是生产 Python 算法（`image_processor_hough.py`，约 6700 行）的**逐模块镜像**。

- 算法逻辑与 Python 对齐，可用于对比验证、性能优化、离线部署
- 通信/调度仍由 Python 负责，C++ 作为计算模块替代品（`scripts/cpp_engine_wrapper.py` 提供调用封装）
- 也可独立运行（单图 / stdin JSON / 批量三种模式）
...
| | Python 引擎 | C++ 引擎（本仓库） |
|---|---|---|
| 角色 | **生产参考实现** | **镜像开发**（核心工作对象） |
| 位置 | `image_processor_hough.py` | `src/`（10 个模块） |
| 流程 | 预处理 → Hough → 直线合并 → 缺陷分类 | 逐模块同构镜像 |
| 验证 | 基准 | 逐图对比缺陷数量，与 Python 一致 |

**镜像方式**：每个 C++ 模块对应 Python 中的一个函数，接口与阈值逐参数对齐（见下方模块映射表）。

**验证方法**：`scripts/batch_compare.py` 对同一图片分别跑 Python / C++，比对缺陷数量与类型@位置。

**当前已知差异**：merge 的 Canny snap（`_snap_line_to_canny`）尚未镜像——影响主边位置 1~7px，计数验证不受影响。

**配置两套**：`hough_inspector_params`（明场，生产使用）/ `hough_inspector_dark_params`（暗场，请求注入 `"mode":"dark"` 切换）。
...
编译产物：静态库 `engine` + 可执行 `glass_engine`。
检测流程：**预处理 → Hough 直线 → 直线合并 → 缺陷分类**。
...
> ⚠️ 源文件列表共 10 个（json_io / preprocess / line_merge / classify / static_artifact / detector / skew / luminosity / corner / main），勿漏。

---

## 使用方式

`glass_engine` 支持三种运行模式：
...
**请求 JSON 支持字段**：`roi_template_file`、`system_params.pixels_per_mm`、`hough_inspector_params`（明场）、`hough_inspector_dark_params`（暗场，需 `"mode":"dark"`）、`draw_defects` + `annotated_output_dir`（标注图输出）、`cam_key`、`static_artifact_enabled`。

Python 调用封装见 `scripts/cpp_engine_wrapper.py`（`CppEngine.detect` / `detect_batch`）。

### 标注图输出（draw_defects）

请求携带 `annotated_output_dir` 时输出 `<原文件名>_annotated.jpg`：

- Q：区域填充 + 射线箭头 + 玻璃轮廓黄线
- L / B / X / E：框填充 + 轮廓
- 文本标注（ASCII 标签，ROI 右上角）

镜像 Python `_draw_defect_annotations`（过滤后重绘，只标注保留的缺陷）。
...
line3 剩余 1 张为 **cam2 数据混入**（cam2 图用 cam1 ROI 验证）：C++ 已报出与 Python 匹配的 E，多出 1 个 E 源于 Canny snap 未镜像；需 cam2 ROI 模板单独验证。
